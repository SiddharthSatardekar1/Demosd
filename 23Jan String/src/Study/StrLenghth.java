package Study;

import java.awt.geom.NoninvertibleTransformException;

public class StrLenghth {

	public static void main(String[] args) {
		//System.out.println("Siddharth");
		//String name = new String("Siddharth");
		String name = "Siddharth";
		//System.out.println(name);
		//int value = name.length();
		//System.out.println(value); 
		
		//String lstring= name.toLowerCase();
		//System.out.println(lstring);
		
		
		//String ustring = name.toUpperCase();
		//System.out.println(ustring);
		
		//String nontrimmedstring = "     Siddharth     ";
		//System.out.println(nontrimmedstring);
		
				// String trimmed =  "      Siddharth           ";
		//String trimmed = nontrimmedstring.trim();
		
		//System.out.println(trimmed);
	//System.out.println(name.substring(2)); 
	//System.out.println(name.substring(5,9));
	
		//	System.out.println(name.replace('h','u'));
		//	
		//	System.out.println(name.replace("rt","ss")); 
		//	
		//	System.out.println(name.replace("d", "bb"));
		
		System.out.println(name.startsWith("Sid"));
		
		System.out.println(name.endsWith("rr"));
		
		System.out.println(name.charAt(1));
		
	}

}
